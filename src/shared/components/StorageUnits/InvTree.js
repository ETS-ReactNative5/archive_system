...
class InvTree extends React.Component{
    ...
    componentDidMount(){
        ...
        this.setState({ tableData: this.renderTableData(this.props.dataInv), tableLoading: false });
    ...
    onSave = () => {
        if (this.state.myValues.length>0 ){
            if (this.props.myValues2.length>0){
                ...
                let caseInventory = {
                    value:this.state.selectedRow.key,
                    idDataPropVal:!!this.props.caseInventory?this.props.caseInventory:null
                }
                copyvall.idDataPropVal=this.props.myValues2[0].idDataPropVal
                this.props.onSave({caseStructuralSubdivision:copyvall,caseInventory:caseInventory,invFund:{value:"",idDataPropVal:this.props.invFund}})
            }else{
                let caseInventory = {
                    value:this.state.selectedRow.key,
                    idDataPropVal:!!this.props.caseInventory?this.props.caseInventory:null
    ...
    render() {
        ...
        const treeColumns = [
            { title: 'Схема описи', dataIndex: 'title', key: 'title', width:'90%' },
...
}, { getObjByProp, getAllObjOfCls })(InvTree);