...
class RenameFormFoundMaker extends React.PureComponent {
    ...
    render(){
        ...
        const lng = localStorage.getItem('i18nextLng');
        const {t,handleSubmit, reset, dirty, error, submitting, tofiConstants: {shortName, dateRename, reasonFundmaker}
        } = this.props;
        ...
        return(
            <div className="FundMaker">
                <div className="table-header-btns" style={{marginTop:'2vw', marginLeft:'1vw'}}>
                            <Button onClick={this.openCard}>{this.props.t('ADD_RENAME')}</Button>
                </div>
...
const selector = formValueSelector('RenameFormFoundMaker');
...
)(reduxForm({form: 'RenameFormFoundMaker', enableReinitialize: true})(RenameFormFoundMaker));