...
import {Button, Icon, Input, Modal, message, Popconfirm, Cascader, Spin} from 'antd';
...
import {
    getPropMeta, onSaveCubeData, parseCube_new,
    parseForTable
} from '../../utils/cubeParser';
import {
    CUBE_FOR_FUND_AND_IK, CUBE_FOR_WORKS,
    DO_FOR_FUND_AND_IK, DO_FOR_WORKS,
...
class FundsList extends Component {
    ...
    getCubeAct = async (ids) => {
            ...
            filterDPAnd: [
                {
                    ...
                    conds: [
                        {
                            consts: 'fundArchive,fundNumber,fundIndex,fundDbeg,fundDend,fundCategory,fundType,fundFeature,isActive'
                        }
                    ]
                }
            ],
                    ...
                    conds: [
                        {
                            ...
                            [doKey]: ids || 'fundOrg,fundLP,collectionOrg,collectionLP,jointOrg,jointLP'
            ...
            filterDTOr: [
                {
                    ...
                    conds: [
                        {
                            ids: this.props.globalDate.startOf('year').format('YYYYMMDD') + this.props.globalDate.endOf('year').format('YYYYMMDD')
                        }
                    ]
                }
            ]
        ...
        await this.props.getCube(CUBE_FOR_FUND_AND_IK, JSON.stringify(filters), {customKey: ids ? 'singleCube' : CUBE_FOR_FUND_AND_IK});
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.singleCube !== this.props.singleCube) {
            ...
            const parsedCube = parseCube_new(this.props.singleCube['cube'], [], 'dp', 'do', this.props.singleCube[`do_${doForFundAndIK.id}`], this.props.singleCube[`dp_${dpForFundAndIK.id}`], `do_${doForFundAndIK.id}`, `dp_${dpForFundAndIK.id}`)
            .map(this.renderTableData);
            this.setState({
                siderCardChild: <FundsListCard
                t={this.props.t}
        ...
        if (!isEmpty(this.props.cubeForFundAndIK) && !isEmpty(this.props.tofiConstants) && this.props.cubeForFundAndIK !== prevProps.cubeForFundAndIK) {
            this.populate();
        }
    }
    ...
    onCreateObj = async ({fundType, shortName, name, accessLevel, ...values}, id, objData) => {
        let hideLoading;
        try {
            const cube = {
            ...
            if (!id) {
                const hideCreateObj = message.loading(this.props.t('CREATING_NEW_OBJECT'), 40);
                await createObj(cube, obj)
                .then(res => {
                    hideCreateObj();
                    if (res.success) {
            ...
            const res = await onSaveCubeData({
                cube,
                obj
            }, {values}, this.props.tofiConstants, objData);
            hideLoading();
            if (res.success) {
    ...
    onFundIndustryObjChange = (v, selOpts) => {
        ...
        this.setState(state => ({filter: {...state.filter, fundIndustryObj: selOpts}}));
    };
    ...
    getFilteredFundsInfo = (ids)=>{
        ...
        var FundsIdFiltered=this.filteredData.map(e => e.key.split('_')[1]).join(",");
        var fundsList = new FormData();
        fundsList.append('fundsList', FundsIdFiltered);
        axios.post(`/${localStorage.getItem('i18nextLng')}/entity/getFilteredFundsInfo`, fundsList).then(response => {
    ...
    renderTableFooter = () => {
        ...
        <div className="table-footer">
            <div className="flex">
                <div className="label"><label htmlFor="">{t('COUNT_FUNDS')}</label><Input
                size='small' type="text" readOnly value={this.filteredData.length}/></div>
                <div className="label"><label htmlFor="">{t('COUNT_INVENT')}</label><Input
                size='small' type="text" readOnly value={countInv}/></div>
                <div className="label"><label htmlFor="">{t('COUNT_CASES')}</label><Input
                size='small' type="text" readOnly value={countDelo}/></div>
                <div className="label"><label
                htmlFor="">{t('COUNT_CASES_WITH_ELECTR')}</label><Input size='small'
                                                                        type="text"
                                                                        readOnly
                                                                        value={countDeloFile}/>
                </div>
                <Button
                type="primary" icon="reload" loading={this.state.iconLoading} onClick={this.getFilteredFundsInfo}
                />
            </div>
    ...
    renderTableData = item => {
        const constArr = [
            'fundDbeg', 'fundDend', 'fundNumber', 'fundIndex', 'fundCategory', 'fundFeature', 'locationOfSupplementaryMaterials',
            'fundIndustry', 'fundmakerOfIK', 'fundmakerMulti', 'fundExitDate', 'fundExitReason', 'fundToGuidbook', 'fundArchive',
            'fundFirstDocFlow', 'fundDateOfLastCheck', 'collectionCreateDate', 'creationConds', 'fundHistoricalNote',"caseStorageMulti","rackMulti" , "sectionMulti", "shelfMulti",
            'creationReason', 'creationPrinciple', 'collectionLocation', 'fundAnnotationFile', 'invFile', 'accessDocument', 'affiliation'
        ];
    ...
    render() {
        ...
        const {
            t, tofiConstants,
            tofiConstants: {fundNumber, fundDbeg, fundDend, fundIndex, fundCategory, fundFeature, fundIndustry, fundArchive, affiliation}
        } = this.props;

        this.filteredData = data.map(this.renderTableData).filter(item => {
            return (
            ( !filter.fundNumber || item.fundNumber == filter.fundNumber ) &&
        ...
        <div className="fundsList" ref={node => this.fundList = node}>
            <div className="fundsList__heading">
                <div className="fundsList__heading-buttons">
                    ...
                    disabled={ isEmpty(selectedRow) || !selectedRow.key }>{t('VIEW_INVENTORIES')}</Button>
                    </Link>
{/*
...
*/}
                ...
                <div className="label-select">
                    ...
                    options={this.props.fundArchiveOptions ? this.props.fundArchiveOptions.map(option => ({
                        value: option.id,
                    ...
                    [
                        ...
                        },
                        {
                            ...
                            <div className="custom-filter-dropdown">
                                <Input
                                name="fundList"
                                suffix={filter.fundList ?
                                <Icon type="close-circle" data-name="fundList"
                                      onClick={this.emitEmpty}/> : null}
                        ...
                        },
                        {
                            ...
                            <div className="custom-filter-dropdown">
                                <Input
                                name="fundDbeg"
                                suffix={filter.fundDbeg ?
                                <Icon type="close-circle" data-name="fundDbeg"
                                      onClick={this.emitEmpty}/> : null}
                        ...
                        },
                        {
                            ...
                            <div className="custom-filter-dropdown">
                                <Input
                                name="fundDend"
                                suffix={filter.fundDend ?
                                <Icon type="close-circle" data-name="fundDend"
                                      onClick={this.emitEmpty}/> : null}
                        ...
                        },
                        {
                            ...
                            render: (text, record) => {
                                ...
                                <div className="editable-row-operations"
                                     style={{display: 'flex'}}>
                                    <Button icon="edit" className="green-btn"
