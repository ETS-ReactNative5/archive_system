...
class Works extends React.PureComponent {
  ...
  constructor(props) {
    ...
    // window.___uses___works___ = this;
    this.state = {
      visible: false,
      data: [],
      nameFilter:"",
  ...
  // updateCard = () => {
  //   // this.changeSelectedRow(this.state.selectedRow);
  //   // window.___uses___works___.changeSelectedRow(window.___uses___works___.state.selectedRow);
  //   me.changeSelectedRow(me.state.selectedRow);
  // }
    ...
    this.getCubeActSingle(rec.key)
      .then(x => {
        ...
        let oneRecord = parseCube_new(
          ...
          'dp',
          'do',
          this.props.cubeForWorksSingle[`do_${doForWorks.id}`],
          this.props.cubeForWorksSingle[`dp_${dpForWorks.id}`],
          `do_${doForWorks.id}`,
          `dp_${dpForWorks.id}`);
              ...
              conds: [
                {
                  ...
                  "workRegCase,workDescription,workStatusCreateRequirement,workStatusForResearches,workRegFund,workRegInv,dateAppointment," +
                  "fundArchive,workRecipient, workStatusCopyDoc,workStatusRegistration,childWorkFeature,linkToWork," +
                  "propResearcheRequests,resultDescription,resultResearch,docsResearch,resultResearchStatus"
                }
              ]
            }
          ]
      };
      return this.props.getCube(CUBE_FOR_WORKS, JSON.stringify(this.filters), {customKey: 'cubeForWorks'});
    }
    catch (err) {
  ...
  getCubeActSingle = async (id) => {
    try {
      ...
      return this.props.getCube(CUBE_FOR_WORKS, JSON.stringify(filters), {
        customKey: 'cubeForWorksSingle',
        parent: id,
      });
    }
    catch (err) {
...
function mapStateToProps(state) {
  const toReturn = {
    ...
    cubeForWorksSingle: state.cubes["cubeForWorksSingle"],
