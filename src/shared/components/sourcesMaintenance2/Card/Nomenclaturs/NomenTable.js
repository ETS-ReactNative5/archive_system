...
class NomenTable extends React.Component {
    ...
    addRow = () => {
        ...
        createObj({cubeSConst: cube.cubeSConst}, obj).then(res => {
            if (res.success) {
                ...
                JSON.stringify(data),
                {},
                {}).then(res2 => res2.success == true ? message.success(this.props.t('PROPS_SUCCESSFULLY_UPDATED'), this.buildComponent()) : message.error(res2.errors[0].text))
    ...
    check = (obj, key, c, type) => {
        ...
        if (type == '41') {
            ...
            const objData = {};
            onSaveCubeData(c, v, this.props.tofiConstants, objData).then(res => res.success == true ? message.success(this.props.t('PROPS_SUCCESSFULLY_UPDATED'), this.buildComponent()) : message.error(res.errors[0].text));
        }
        let uniq = '1';
        let editable = {...this.state.editable};
    ...
    delFile = async file => {
        ...
        const res = await dFile(file.name, 'cubeSNomen');
        if (!res.success) {
    ...
    render() {
        ...
        <div>
            ...
            <Button type='primary' disabled={!this.state.selectedRow}
