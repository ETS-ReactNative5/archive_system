...
import { Button, Table as AntTable, Input, Checkbox, InputNumber, Icon, message, Popconfirm, DatePicker, Modal } from 'antd'
...
import {createObj, getAllObjOfCls, getCube, updateCubeData, getObjByProp, getObjByObjVal, getPropVal, dObj} from "../../actions/actions";
...
class CreateDocument extends React.Component {
  ...
  onChangeFundmakerArchive = (item) => {
    ...
    this.setState({
      filter: { ...filter, fundmakerArchive: item, fundType: null, fundOrg: null, invList: null, caseList: null },
      fundOrgOptions: [],
      invListOptions: [],
      caseListOptions: [],
      tableData: [],
  ...
  onChangeFundType = (item) => {
    const { filter } = this.state;
    this.setState({
      filter: { ...filter, fundType: item, fundOrg: null, invList: null, caseList: null },
      fundOrgOptions: [],
      invListOptions: [],
      caseListOptions: [],
      tableData: [],
  ...
  renderTableData = (item,idx) => {
    const { invNumber, pageNumberStart, turnoverSheetStart, pageNumberEnd, turnoverSheetEnd, start, end,
            ...
            linkToKatalog, linkToUkaz, linkToObzor } = this.props.tofiConstants;
...
// complex
    ...
    return {
      ...
      //
      invNumber: !!invNumberObj && invNumberObj.value ? invNumberObj.value : invNumberObj.value,
      orderIndex: parseInt(!!invNumberObj && invNumberObj.value ? invNumberObj.value : '0') * 1000,
      //start, end
      pageNumberStart: startObj && startObj.idDataPropVal && pageNumberStartObj && pageNumberStartObj.complexChildValues[startObj.idDataPropVal] && pageNumberStartObj.complexChildValues[startObj.idDataPropVal].value,
      turnoverSheetStart: startObj && startObj.idDataPropVal && turnoverSheetStartObj && turnoverSheetStartObj.complexChildValues[startObj.idDataPropVal] && turnoverSheetStartObj.complexChildValues[startObj.idDataPropVal].refId,
      pageNumberEnd: endObj && endObj.idDataPropVal && pageNumberEndObj && pageNumberEndObj.complexChildValues[endObj.idDataPropVal] && pageNumberEndObj.complexChildValues[endObj.idDataPropVal].value,
      turnoverSheetEnd: endObj && endObj.idDataPropVal && turnoverSheetEndObj && turnoverSheetEndObj.complexChildValues[endObj.idDataPropVal] && turnoverSheetEndObj.complexChildValues[endObj.idDataPropVal].refId,
      //pageNumberEnd
      //turnoverSheetEnd
      documentPapers: !!documentPapersObj && documentPapersObj.value ? documentPapersObj.value : '',
      dateForming: !!dateFormingObj && dateFormingObj.value ? moment(dateFormingObj.value, 'DD-MM-YYYY') : null,
      ...
      surnameOriginator: !!surnameOriginatorObj && surnameOriginatorObj.prop ? {value:surnameOriginatorObj.prop, label:surnameOriginatorObj.value} : null,
      //
      documentType: !!documentTypeObj && documentTypeObj.refId ? {value:documentTypeObj.refId, label: documentTypeObj.value} : null,
      fundOrg: !!fundOrgObj && fundOrgObj.refId ? {value:fundOrgObj.refId, label:fundOrgObj.value} : null,
      fundLP: !!fundLPObj && fundLPObj.refId ? {value:fundLPObj.refId, label:fundLPObj.value} : null,
      ...
      //
      documentAuthor: !!documentAuthorObj ? documentAuthorObj.value : '',
      addressee: !!addresseeObj ? addresseeObj.value : '',
      question: !!questionObj ? questionObj.value || '' : '',
      event: !!eventObj ? eventObj.value || '' : '',
      person: !!personObj ? personObj.value : [],
      //
      nameOrPrimaryWords: !!nameOrPrimaryWordsObj ? nameOrPrimaryWordsObj.value || '' : '',
      documentContent: !!documentContentObj ? documentContentObj.value || '' : '',
      peopleMentioned: !!peopleMentionedObj ? peopleMentionedObj.value || '' : '',
      organizationsMentioned: !!organizationsMentionedObj ? organizationsMentionedObj.value || '' : '',
      documentKeywords: !!documentKeywordsObj ? documentKeywordsObj.value || '' : '',
      //
      propAuthenticity: !!propAuthenticityObj && propAuthenticityObj.refId ? {value:propAuthenticityObj.prop, label:propAuthenticityObj.value} : null,
      typeOfPaperCarrier: !!typeOfPaperCarrierObj && typeOfPaperCarrierObj.refId ? {value:typeOfPaperCarrierObj.prop, label:typeOfPaperCarrierObj.value} : null,
      //
      objectCode: !!objectCodeObj && objectCodeObj.value ? objectCodeObj.value : '',  
      projectName: !!projectNameObj && projectNameObj.value ? projectNameObj.value : '',  
      projectStage: !!projectStageObj && projectStageObj.value ? projectStageObj.value : '',  
      projectPartName: !!projectPartNameObj && projectPartNameObj.value ? projectPartNameObj.value : '',  
      drawingName: !!drawingNameObj && drawingNameObj.value ? drawingNameObj.value : '',  
      volumeNumber: !!volumeNumberObj && volumeNumberObj.value ? volumeNumberObj.value : '',  
      documentNTDFIO: !!documentNTDFIOObj && documentNTDFIOObj.value ? documentNTDFIOObj.value : '',  
      yearOfCompletion: !!yearOfCompletionObj && yearOfCompletionObj.value ? yearOfCompletionObj.value : '',  
      //
      ...
      //
      //inaccurateDate 
      inaccurateDateFeature: !!inaccurateDateFeatureObj ? inaccurateDateFeatureObj.value : '',
...
*/
  ...
  save(key, parentKey) {
    ...
    switch (filter.fundType.value) {
      case 'fundOrg': case 'collectionOrg': case 'joinOrg':
        provenance = 'fundOrg'; break;
      case 'fundLP': case 'collectionLP': case 'joinLP':
        provenance = 'fundLP'; break;
    }

    const name = {};
    const cipherDocument = `${target.pageNumberStart}-${target.pageNumberEnd}`;
    const archiveCipher = {};
    const documLang = {kz: ', іс парақтары ', ru: ', страницы дела ', en: ', pages of case '};
    ['kz', 'ru', 'en'].map(lang => {
    ...
    const documentPapers = String(parseInt(pageNumberEnd) - parseInt(pageNumberStart) + 1 );
    
    const dateForming = dateFormingVal ? moment(dateFormingVal, 'DD-MM-YYYY').format('MM-DD-YYYY'): dateFormingVal;

    let fragmentRest = {};
    if (this.state.newFragment) {
      fragmentRest = {
        //
        dateForming: parentObj.dateForming ? parentObj.dateForming.format('MM-DD-YYYY') : undefined,
        //
    ...
    createObj(cube, obj)
      .then(res => {
        ...
        if(res.success) {
          ...
          const { pageNumberStart, turnoverSheetStart: turnoverSheetStartVal, pageNumberEnd, turnoverSheetEnd: turnoverSheetEndVal, 
                  nomenLastChangeDate, dateForming: dateFormingVal } = target;
          const turnoverSheetStart = turnoverSheetStartVal ? String(this.props.tofiConstants.yes.id) : String(this.props.tofiConstants.no.id)
          const turnoverSheetEnd = turnoverSheetEndVal ? String(this.props.tofiConstants.yes.id) : String(this.props.tofiConstants.no.id)
          const dateForming = dateFormingVal ? moment(dateFormingVal, 'DD-MM-YYYY').format('MM-DD-YYYY'): dateFormingVal;
          const cube = {
            cubeSConst: 'cubeDocuments',
            doConst: 'doDocuments',
          ...
          //console.log(complex)
          const values = { 
            invNumber, documentPapers, nomenLastChangeDate, archiveCipher, provenance, 
            dateForming, surnameOriginator, documentCase: String(this.state.filter.caseList.value), ...fragmentRest
          }
  ...
  changeSelectedRow = (rec, openAtOnce) => {
    ...
    if (openAtOnce === true) {
      this.setState({ selectedRow: {...rec, extraInfo}, changedRow: {...rec, extraInfo}, selectedRowKey: rec.key, openCard: true, sidebarActiveKey: 'header' })
      return;
    }
    
    this.setState({ selectedRow: {...rec, extraInfo}, changedRow: {...rec, extraInfo}, selectedRowKey: rec.key })
  };
  ...
  onEditRecordChange = (label, data) => {
    ...
    if (typeof label !== 'string' && label.documentFile) {
      ...
      switch (label) {
        case 'reference_clsKatalog':  changedRow.linkToKatalog = data;  break;
        case 'reference_clsUkaz':     changedRow.linkToUkaz = data;     break;
        case 'reference_clsObzor':    changedRow.linkToObzor = data;    break;
        default: changedRow[label] = data;
      }
    }
    this.setState({
  ...
  refreshRecord = () => {
    ...
    return this.onSaveCubeData({cube}, {...values, surnameOriginator, nomenLastChangeDate}, this.state.selectedRowKey, {}, this.state.selectedRow)
  };
  ...
  render() {
    const { tableData, selectedRow, openNewRec, openCard, filterLoading, filter, 
            fundOrgOptions, invListOptions, caseListOptions } = this.state;
        ...
        <div className="CreateDocument__heading">
          <div className="table-header">
            ...
            <div className="label-select-30">
              <SelectVirt
                ...
                onFocus={() => {
                  this.setState({filterLoading: { ...this.state.filterLoading, fundOrg: true }});
                  ...
                  getObjByProp(fd)
                    .then(res => {
                      ...
                      this.setState({filterLoading: { ...this.state.filterLoading, fundOrg: false }});
                    }).catch(err => console.log(err))
                }}
            ...
            <div className="label-select-30">
              <SelectVirt 
                ...
                onMenuOpen={() => {
                  this.setState({filterLoading: { ...this.state.filterLoading, caseList: true }});
                  const fd = new FormData();
                  ...
                  getObjByProp(fd)
                    .then(res => {
                      ...
                      this.setState({filterLoading: { ...this.state.filterLoading, caseList: false }});
                    }).catch(err => console.log(err))
                }}
          ...
          <div className="table-header">
            <div className="label-select-10" style={{flex: 1}}>
              <Button disabled={disabledAddDocument} onClick={this.addNewDocument}>{t('ADD_DOCUMENT')}</Button>
              <Button disabled={disabledAddFragment} onClick={this.addNewFragment}>{t('ADD_FRAGMENT')}</Button>
            </div>
          </div>
        </div>
        <div className="CreateDocument__body">
                    ...
                    <div className="editable-row-operations">
                      {
                        editable ?
                          <span>
                            <a onClick={() => this.save(record.key, record.parent)}><Icon type="check"/></a>
                            <Popconfirm title="Отменить?" onConfirm={() => this.cancel(record.key, record.parent)}>
          ...
          >
            <SiderCard 
              ...
              onTabClick={this.onSideBarTabClick} activeKey={this.state.sidebarActiveKey}
