...
import {Button, Icon, Input, Modal, message, DatePicker} from 'antd';
...
*/
  ...
  onFundIndustryObjChange = s => {
    ...
    this.setState({filter: {...this.state.filter, fundIndustryObj: s, fundIndustryObjChildren: getLastChildren(this.props.fundIndustryObjOptions, s)}})
  ...
  renderTableData = (item,idx) => {
    const { fundDbeg, fundDend, fundNumber, fundIndex, fundCategory, fundFeature,
    ...
    return {
      ...
      fundmakerOfIK: fundmakerOfIKObj && fundmakerOfIKObj.cube && fundmakerOfIKObj.cube.idRef ? {value: fundmakerOfIKObj.cube.idRef, label: fundmakerOfIKObj.value} : {},
  ...
  render() {
    ...
    this.filteredData = data.map(this.renderTableData).filter(item => {
      return (
        ...
        String(item.fundNumber.toLowerCase()).includes(String(filter.fundNumber.toLowerCase())) &&
        ...
        <div className="fundsList__heading">
          ...
          <div className="label-select">
            <Select
              ...
              options={this.props.fundCategoryOptions ? this.props.fundCategoryOptions.map(option => ({value: option.id, label: option.name[this.lng]})) : []}
          ...
          <div className="label-select">
            <Select
              ...
              options={
                ['fundOrg','fundLP','collectionOrg','collectionLP','jointOrg','jointLP']
                  .map(c => ({ value: this.props.tofiConstants[c].id, label: this.props.tofiConstants[c].name[this.lng] }))
              }
          ...
          <div className="label-select">
            <Select
              ...
              options={this.props.fundFeatureOptions ? this.props.fundFeatureOptions.map(option => ({value: option.id, label: option.name[this.lng]})) : []}
                  ...
                  width: "8%",
                    filterDropdown: (
                        <div className="custom-filter-dropdown">
                            <Input
                                name="fundNumber"
                                suffix={filter.fundNumber ? <Icon type="close-circle" data-name="fundNumber" onClick={this.emitEmpty} /> : null}
