...
export const renderInputLang = ({input, formItemClass, meta: {submitFailed, error}, formItemLayout, changeLang, colon, ...rest}) => {
    ...
    >
        ...
        <RadioGroup onChange={e => {
            changeLang(e);
            inpRef && inpRef.focus()
        }} defaultValue={localStorage.getItem('i18nextLng')} name={input.name}>
...
export const renderDoubleDatePicker = fields => {
    ...
    >
        <div className="flex">
            ...
            />
            {submitFailedFirst && (errorFirst &&
            <span className="message-error">{errorFirst}</span>)}
            <DatePicker
            format="DD-MM-YYYY"
            {...inputSecond}
            {...restSecond}
        ...
        {submitFailedSecond && (errorSecond &&
        <span className="message-error">{errorSecond}</span>)}
    </FormItem>
    )
};
